//From the desk of Frank P. Westlake; the public domain.
...
public class FSNavigator
    ...
    private LinearLayout directoryEntry(final String name) {
        ...
        if (up) {
            ...
            if (countDirectoryView < cachedDirectoryView.size()) {
                ll = cachedDirectoryView.get(countDirectoryView);
            } else {
                ll = directoryView(up);
                cachedDirectoryView.put(countDirectoryView, ll);
            }
            ++countDirectoryView;
        }
    ...
    private void makeView() {
        ...
        String[] zd = cd.list(new java.io.FilenameFilter() {
        ...
        String[] zf = cd.list(new java.io.FilenameFilter() {
            public boolean accept(File file, String name) {
                return (!(new File(file, name).isDirectory()));
            }
        });
